class PssmJsonController < ApplicationController
  ...
  def pssm_ch_rest
    ...
    db.execute( "select * from pdbsTable where pdb=\"#{pdb.downcase}\" and chain=\"#{chain}\"" ) do |r|
      ...
      db.execute( "select stepNum,status from pdbChainIterStatus where pdb=\"#{pdb.downcase}\" and chain=\"#{chain}\";" ) do |s|
        statuts[ s[0]-2 ] = s[1]
      end
      if statuts[0] == 0
  ...
  def get_pssm(pdb,ch,n)
    cmd = "unzip -p "+PDB_PSSM_PATH+"/compressed/"+pdb+"_"+ch+"_"+n+".pssm.zip"
    file = `#{cmd}`
    out_res = []
    out_scores = []
    index = 1
